...
void physics(void)
{
    if (gl.walk || gl.keys[XK_d] || gl.keys[XK_a]) {
        //man is walking...
...
void render(void)
{
    ...
    //if (gl.walkFrame >= 8)
    //	iy = 1;
    float fx = (float)ix / 5.0;
    float fy = (float)iy / 3.0;
    glBegin(GL_QUADS);
    if (gl.keys[XK_a]) {
        glTexCoord2f(fx, fy);
        glVertex2i(cx-w, cy-h);

        glTexCoord2f(fx+.5, fy);  
        glVertex2i(cx-w, cy+h);
        ...
        /*         printf("%f cx, %f w\n", cx, w);
                   printf("%f cy, %f h\n", cy, h);
                   glTexCoord2f(fx,      fy+.125); glVertex2i(cx-w, cy-h);
                   glTexCoord2f(fx+.5,      fy);    glVertex2i(cx-w, cy+h);
                   glTexCoord2f(fx, fy+.5);    glVertex2i(cx+w, cy+h);
                   glTexCoord2f(fx, fy+.5); glVertex2i(cx+w, cy-h);*/
        glTexCoord2f(fx,      fy); glVertex2i(300, 100);
        glTexCoord2f(fx+.5,      fy);    glVertex2i(300, 500);
        glTexCoord2f(fx, fy+.5);    glVertex2i(500, 500);
        glTexCoord2f(fx+.5, fy); glVertex2i(500, 100);
    }
    glEnd();
    glPopMatrix();
